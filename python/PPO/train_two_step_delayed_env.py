...
from test_envs_basics import TwoStepDelayedRewardEnv


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--env", default="TwoStepDelayedRewardEnv")
    p.add_argument("--timesteps", type=int, default=10_000)
    p.add_argument("--logdir", default="runs/two_step_delayed_reward_ppo_clip")
    p.add_argument("--save_path", default="runs/two_step_delayed_reward_ppo_clip/final.pt")
...
def make_env(env_name: str, seed: int) -> gym.Env:
    ...
    if env_name == "TwoStepDelayedRewardEnv":
...
def main():
    ...
    # If logdir is relative and doesn't already include python/PPO/, prepend it
    logdir_rel = args.logdir
    if not os.path.isabs(logdir_rel) and not logdir_rel.startswith("python/PPO/"):
        logdir_rel = f"python/PPO/{logdir_rel}"
    print("\n" + "="*70)
    ...
    # Configure PPO for TwoStepDelayedRewardEnv (simple environment, smaller network is fine)
    # Note: This environment has delayed reward (reward only at step 2), so gamma is important
    cfg = PPOConfig(
        gamma=0.99,
        gae_lambda=0.95,
        clip_range=0.2,
    ...
    # Track solved status
    # TwoStepDelayedRewardEnv: Step 0 gives reward=0, Step 1 gives reward=1
    # Episode takes 2 steps, reward is delayed until the end
    # We consider it solved if the agent gets the reward (1.0)
    ...
    print("  - TwoStepDelayedRewardEnv: Episode takes 2 steps")
    print("  - Step 0: obs=0, reward=0, not terminated")
    print("  - Step 1: obs=1, reward=1, terminated")
    print("  - Agent must learn to continue through step 0 to get delayed reward at step 1")
    print("-" * 50)
