...
public class RLBridgeServer : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (resetRequestedEvent.WaitOne(0))
        {
            ...
            resetResponseJson = BuildResponseJson(obs, 0f, false, 0f);
            resetCompleted.Set();
        ...
        if (stepRequested.WaitOne(0))
        {
            try
            {
                if (controlledCar == null)
                {
                    ...
                    stepResponseJson = BuildResponseJson(new float[] { 0f }, 0f, true, 0f);
                    stepCompleted.Set();
                    return;
                }
                ...
                stepResponseJson = BuildResponseJson(obs, reward, done, prevCompletion);
            ...
            catch (System.Exception ex)
            {
                ...
                stepResponseJson = BuildResponseJson(new float[] { 0f }, 0f, false, 0f);
                stepCompleted.Set();
            }
        }
        // if (stepRequested.WaitOne(0))
    ...
    private string BuildResponseJson(float[] obs, float reward, bool done, float completion)
    {
        ...
        sb.Append(completion.ToString(System.Globalization.CultureInfo.InvariantCulture));
        // END NEWW
        sb.Append('}');
        return sb.ToString();
    }
